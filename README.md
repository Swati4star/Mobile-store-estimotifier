...
###2. Display Services
As soon as the estimote is detected by the app, the services offered by that store are downloaded from the cloud and presented to the user. When a user selects some service, he is provided with some unique token number, and an estimated waiting time. The main benefit of this is, that the user need not to wait in the queue. He just needs to show his token number.
